...
async def seeded(client: AsyncClient, auth_headers: dict) -> None:
    ...
    account = (await client.post("/api/v1/accounts", json={"name": "Checking", "type": "checking"}, headers=auth_headers)).json()
    category = (await client.post("/api/v1/categories", json={"name": "Food"}, headers=auth_headers)).json()

    await client.post("/api/v1/transactions", json={
        "account_id": account["id"], "category_id": category["id"],
        "type": "income", "amount": "1000.00", "date": "2024-03-10T00:00:00Z",
    }, headers=auth_headers)
    await client.post("/api/v1/transactions", json={
        "account_id": account["id"], "category_id": category["id"],
        "type": "expense", "amount": "200.00", "date": "2024-03-15T00:00:00Z",
    }, headers=auth_headers)
...
async def test_categories_report_empty(client: AsyncClient, auth_headers: dict):
    resp = await client.get(f"{BASE}/categories", params={"month": "2024-03-01"}, headers=auth_headers)
    assert resp.status_code == 200
...
async def test_cashflow_report_empty(client: AsyncClient, auth_headers: dict):
    resp = await client.get(f"{BASE}/cashflow", params={"date_from": "2024-03-01", "date_to": "2024-03-31"}, headers=auth_headers)
    assert resp.status_code == 200
