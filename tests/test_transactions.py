...
async def transaction(
    client: AsyncClient, auth_headers: dict, account: dict, category: dict
) -> dict:
    resp = await client.post(
        BASE,
        json={
...
async def test_create_transaction_unknown_account(
    client: AsyncClient, auth_headers: dict, category: dict
):
    resp = await client.post(
        BASE,
        json={
...
async def test_update_transaction_description(
    client: AsyncClient, auth_headers: dict, transaction: dict
):
    resp = await client.patch(
        f"{BASE}/{transaction['id']}",
        json={"description": "Weekly groceries"},
        headers=auth_headers,
    )
    assert resp.status_code == 200
...
async def test_delete_transaction(client: AsyncClient, auth_headers: dict, transaction: dict):
    ...
    assert (
        await client.get(f"{BASE}/{transaction['id']}", headers=auth_headers)
    ).status_code == 404
